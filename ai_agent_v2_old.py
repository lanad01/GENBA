...
class DataAnayticsAssistant:
    ...
    def build_graph(self):
        ...
        # ✅ 전체 데이터 실행 후 조건부 라우팅 추가
        workflow.add_conditional_edges(
            "Execute_Full",
            lambda state: "Save_Data" if "analytic_result" in state else "Regenerate_Code",
            {
                "Save_Data": "Save_Data",
                "Regenerate_Code": "Regenerate_Code"
            }
    ...
    def generate_chart(self, state):
        ...
        try:
            ...
            return Command(update={"chart_filename": filename, "retry_chart": 0}, goto=self.route_after_chart({"chart_filename": filename, "retry_chart": 0}))
 
        except Exception as e:
            print(f"❌ 차트 코드 실행 중 오류 발생: {e}")
            plt.close()
            # 실패 시 retry_chart 증가
            next_state = {"chart_filename": None, "retry_chart": retry_cnt + 1}
            return Command(update=next_state, goto=self.route_after_chart(next_state))
