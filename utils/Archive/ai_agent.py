...
class AIAnalysisAssistant:
    ...
    def chart_builder(self, state: State) -> Command:
        ...
        if "```python" in chart_code:
            ...
            print("📈 [create_chart] 유효한 파이썬 코드 블록이 없습니다.")
            return Command(update={"chart_filename": None}, goto="Supervisor")
    ...
    def build_graph(self):
        ...
        # workflow.add_edge("Insight_Builder", "Chart_Builder")
        # workflow.add_edge("Chart_Builder", "Supervisor")

        # ✅ 조건부 엣지 추가: 차트 생성 여부에 따라 흐름 제어
        def chart_is_none(state):
        ...
        # ✅ 조건부 엣지 추가: 차트 필요 여부에 따라 흐름 제어
        def chart_needed(state):
            # print(f"📈 [chart_needed?] Chart decision  {state.get("chart_decision", "")}")
        ...
        workflow.add_conditional_edges(
            ...
            chart_needed,
            {
                False: "Supervisor",   # 차트가 필요 있으면 Chart_Builder로
                True: "Chart_Builder"  # 차트가 필요 없으면 Supervisor로
            },
        )
        ...
        workflow.add_conditional_edges(
            ...
            chart_is_none,
            {
                True: "Supervisor",       # 차트가 없으면 Supervisor로
                False: "Report_Builder"   # 차트가 있으면 Report_Builder로
            },
        )
    ...
    def ask(self, query: str):
        print(f"질문: {query}")
        return self.app.invoke({"messages": [HumanMessage(content=query)]}, config={"recursion_limit": recursion_limit})
