...
def analyze_multiple_dataframes(dataframe_list):
    result_df = pd.DataFrame()  # 최종 결과를 저장할 데이터프레임
        ...
        with pd.ExcelWriter(output_path) as writer:
            for df_name in dataframe_list:
                ...
                if df is not None:
                    ...
                    rslt.to_excel(writer, sheet_name=df_name, index=False)
                    print(f"✅ [LOG] {df_name} 분석 결과가 저장되었습니다.")
                    result_df = pd.concat([result_df, rslt], ignore_index=True)
