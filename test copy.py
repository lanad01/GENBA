...
class DataAnayticsAssistant:
    ...
    def build_graph(self):
        ...
        workflow.add_node("Check_EDA_Question", self.check_eda_question)  # ✅ 추가
        ...
        # ✅ Check_EDA_Question 결과에 따라 분기
        workflow.add_conditional_edges(
            "Check_EDA_Question",
            lambda state: "Eda_Generate_Code" if state.get("eda_question", False) else "Generate_Code",  # ✅ 안전한 접근 방식
            {
    ...
    def check_eda_question(self, state: State) -> Command:
        """사용자의 질문이 EDA 관련인지 판단하는 노드"""
        print("=" * 100)
        print("🔍 EDA 관련 질문 여부 판단 단계")
        ...
        print(f"📌 EDA 관련 질문 여부: {eda_decision}")

        # ✅ 항상 eda_question 값을 설정 (기본값 False 포함)
    ...
    def generate_eda_code(self, state: State) -> Command:
        ...
        #     print("⚠️ 이미 'generated_code'가 존재합니다. 업데이트하지 않습니다.")
